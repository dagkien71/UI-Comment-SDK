(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.CommentSDK = {}));
    ...
    class DebugIcon {
      ...
      `;
        ...
        updateState(isActive) {
            ...
            this.element.className = `uicm-debug-icon uicm-glass uicm-glass-hover ${isActive ? "uicm-active" : ""}`;
    ...
    `;
        ...
        updateTabStates() {
            const activeTab = this.element.querySelector(".uicm-sidebar-tab:first-child");
            const archiveTab = this.element.querySelector(".uicm-sidebar-tab:last-child");
    ...
    class CommentModal {
        ...
        createElement() {
            ...
            commentCount.textContent = `${this.props.comments.length} comment${this.props.comments.length !== 1 ? "s" : ""}`;
            // Status display
            ...
            if (mainComment.status === exports.CommentStatus.ARCHIVED) {
                const option = document.createElement("option");
                option.value = exports.CommentStatus.ARCHIVED;
            ...
            statusDropdown.addEventListener("change", async (e) => {
                ...
                if (newStatus !== mainComment.status) {
                    ...
                    archiveButton.style.display =
                        newStatus === exports.CommentStatus.DONE ? "inline-block" : "none";
                }
            });
            ...
            archiveButton.style.display =
                mainComment.status === exports.CommentStatus.DONE ||
                    mainComment.status === exports.CommentStatus.ARCHIVED
                    ? "inline-block"
                    : "none";
            archiveButton.onclick = async () => {
                await this.props.onStatusChange(mainComment.id, exports.CommentStatus.ARCHIVED);
                statusBadge.textContent = exports.CommentStatus.ARCHIVED.toUpperCase();
                statusBadge.className = `uicm-status-badge status-${exports.CommentStatus.ARCHIVED}`;
                // Update dropdown to show archived status
                statusDropdown.value = exports.CommentStatus.ARCHIVED;
  ...
  `;
    ...
    class CommentSDK {
            ...
            try {
                ...
                this.settingsButton = new SettingsButton({
                    ...
                    onUserUpdate: async (updatedUser) => {
                        ...
                        userProfileStorage.saveUserProfile(updatedUser);
                        console.log("✅ User profile updated in SDK and localStorage:", updatedUser);
                    },
        ...
        navigateToComment(comment) {
            console.log("🔧 CommentSDK: navigateToComment called for comment:", comment.id);
            ...
            setTimeout(() => {
                this.isNavigatingFromSidebar = false;
                window.uicmIsNavigatingFromSidebar = false;
                console.log("🔧 CommentSDK: Navigation flag reset");
            }, 2000);
        }
        highlightCommentOnPage(comment) {
            console.log("🔧 CommentSDK: highlightCommentOnPage called for comment:", comment.id);
            // Switch to comment mode
            this.setMode("comment");
            // Find the element by XPath
            const element = this.findElementByXPath(comment.xpath);
            if (element) {
                console.log("🔧 CommentSDK: Element found, highlighting and scrolling to it");
                ...
                setTimeout(() => {
                    ...
                    if (bubble) {
                        ...
                        console.warn("🔧 CommentSDK: Bubble not found for comment:", comment.id);
                ...
                console.warn("🔧 CommentSDK: Element not found for comment:", comment.xpath);
            }
        }
            ...
            if (listeners) {
                listeners.forEach((callback) => {
                    try {
                        ...
                        console.error(`CommentSDK: Error in event listener for ${event}:`, error);
                    }
                });
            }
        }
